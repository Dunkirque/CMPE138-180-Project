...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($checkResultEmployee->num_rows === 0) {
        ...
        if ($checkResultNewEmployee->num_rows > 0) {
            ...
            // Update data in the AdminEmp table
            $updateQueryAdminEmp = "UPDATE AdminEmp SET EmpSSN = '$newEmpSSN' WHERE EmpSSN = '$oldEmpSSN'";
            if ($mysqli->query($updateQueryAdminEmp) === TRUE) {
                ...
                echo "Error updating records in AdminEmp table: " . $mysqli->error;
            }
        }
    }
}
...
<head>
    ...
    <title>Update Admin Employee Data</title>
</head>
<body>

    <h2>Update Admin Employee Data</h2>

    <form method="post" action="">
        <label for="oldEmpSSN">Enter Existing Admin Employee SSN to Update:</label>
        <input type="text" name="oldEmpSSN" value="<?php echo $oldEmpSSN; ?>" required><br>

        <label for="newEmpSSN">Enter New Admin Employee SSN:</label>
        <input type="text" name="newEmpSSN" value="<?php echo $newEmpSSN; ?>" required><br>

        <label for="newEmpFname">Enter New Admin Employee First Name:</label>
        <input type="text" name="newEmpFname" value="<?php echo $newEmpFname; ?>" required><br>

        <label for="newEmpLname">Enter New Admin Employee Last Name:</label>
        <input type="text" name="newEmpLname" value="<?php echo $newEmpLname; ?>" required><br>

        <label for="newEmpNumber">Enter New Admin Employee Number:</label>
    ...
    <a href="AdminEmp.php">
        <button type="button">Go back to Admin Employee Data</button>
    </a>
</body>
</html>
