...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($checkResultEmployee->num_rows === 0) {
        ...
        if ($checkResultNewEmployee->num_rows > 0) {
            ...
            // Update data in the Inspector table
            $updateQueryInspector = "UPDATE Inspector SET EmpSSN = '$newEmpSSN' WHERE EmpSSN = '$oldEmpSSN'";
            if ($mysqli->query($updateQueryInspector) === TRUE) {
                ...
                echo "Error updating records in Inspector table: " . $mysqli->error;
            }
        }
    }
}
...
<head>
    ...
    <title>Update Inspector Employee Data</title>
</head>
<body>

    <h2>Update Inspector Employee Data</h2>

    <form method="post" action="">
        <label for="oldEmpSSN">Enter Existing Inspector Employee SSN to Update:</label>
        <input type="text" name="oldEmpSSN" value="<?php echo $oldEmpSSN; ?>" required><br>

        <label for="newEmpSSN">Enter New Inspector Employee SSN:</label>
        <input type="text" name="newEmpSSN" value="<?php echo $newEmpSSN; ?>" required><br>

        <label for="newEmpFname">Enter New Inspector Employee First Name:</label>
        <input type="text" name="newEmpFname" value="<?php echo $newEmpFname; ?>" required><br>

        <label for="newEmpLname">Enter New Inspector Employee Last Name:</label>
        <input type="text" name="newEmpLname" value="<?php echo $newEmpLname; ?>" required><br>

        <label for="newEmpNumber">Enter New Inspector Employee Number:</label>
