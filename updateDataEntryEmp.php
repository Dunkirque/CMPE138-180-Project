...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($checkResultEmployee->num_rows === 0) {
        ...
        if ($checkResultNewEmployee->num_rows > 0) {
            ...
            // Update data in the DataEntryEmp table
            $updateQueryDataEntryEmp = "UPDATE DataEntryEmp SET EmpSSN = '$newEmpSSN' WHERE EmpSSN = '$oldEmpSSN'";
            if ($mysqli->query($updateQueryDataEntryEmp) === TRUE) {
                ...
                echo "Error updating records in DataEntryEmp table: " . $mysqli->error;
            }
        }
    }
}
...
<head>
    ...
    <title>Update Data Entry Employee Data</title>
</head>
<body>

    <h2>Update Data Entry Employee Data</h2>

    <form method="post" action="">
        <label for="oldEmpSSN">Enter Existing Data Entry Employee SSN to Update:</label>
        <input type="text" name="oldEmpSSN" value="<?php echo $oldEmpSSN; ?>" required><br>

        <label for="newEmpSSN">Enter New Data Entry Employee SSN:</label>
        <input type="text" name="newEmpSSN" value="<?php echo $newEmpSSN; ?>" required><br>

        <label for="newEmpFname">Enter New Data Entry Employee First Name:</label>
        <input type="text" name="newEmpFname" value="<?php echo $newEmpFname; ?>" required><br>

        <label for="newEmpLname">Enter New Data Entry Employee Last Name:</label>
        <input type="text" name="newEmpLname" value="<?php echo $newEmpLname; ?>" required><br>

        <label for="newEmpNumber">Enter New Data Entry Employee Number:</label>
    ...
    <a href="DataEntryEmp.php">
        <button type="button">Go back to Data Entry Employee Data</button>
    </a>
</body>
</html>
