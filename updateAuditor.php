...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($checkResultEmployee->num_rows === 0) {
        ...
        if ($checkResultNewEmployee->num_rows > 0) {
            ...
            // Update data in the Auditor table
            $updateQueryAuditor = "UPDATE Auditor SET EmpSSN = '$newEmpSSN' WHERE EmpSSN = '$oldEmpSSN'";
            if ($mysqli->query($updateQueryAuditor) === TRUE) {
                ...
                echo "Error updating records in Auditor table: " . $mysqli->error;
            }
        }
    }
}
...
<head>
    ...
    <title>Update Auditor Employee Data</title>
</head>
<body>

    <h2>Update Auditor Employee Data</h2>

    <form method="post" action="">
        <label for="oldEmpSSN">Enter Existing Auditor Employee SSN to Update:</label>
        <input type="text" name="oldEmpSSN" value="<?php echo $oldEmpSSN; ?>" required><br>

        <label for="newEmpSSN">Enter New Auditor Employee SSN:</label>
        <input type="text" name="newEmpSSN" value="<?php echo $newEmpSSN; ?>" required><br>

        <label for="newEmpFname">Enter New Auditor Employee First Name:</label>
        <input type="text" name="newEmpFname" value="<?php echo $newEmpFname; ?>" required><br>

        <label for="newEmpLname">Enter New Auditor Employee Last Name:</label>
        <input type="text" name="newEmpLname" value="<?php echo $newEmpLname; ?>" required><br>

        <label for="newEmpNumber">Enter New Auditor Employee Number:</label>
